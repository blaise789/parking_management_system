...
console.log(parseInt(appConfig().files.uploadLimit))
export const profilePictureUploadConfig = {
  preservePath: true,
  fileFilter: (_req: Request, _file: Express.Multer.File, cb: (error: Error | null, accept: boolean) => void) => {
    cb(null, true);
  },
  storage: diskStorage({
